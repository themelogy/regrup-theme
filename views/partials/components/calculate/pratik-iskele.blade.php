@if(isset($tag))
    <{{ $tag }} class="uppercase motive"><strong>{{ $page->title }}</strong></{{ $tag }}>
@else
<h4 class="uppercase motive"><strong>PRATİK İSKELE HESABI</strong></h4>
@endif
...
<form id="form1" class="form-horizontal" name="form1" method="post">
    ...
    <div class="control-group">
        <div class="controls">
            <button type="button" onclick="pratik_calc('#form1');" class="btn btn-brick btn-xs large-padding pull-left"
                    style="margin-right: 2px;">Hesapla
            </button>
            <button type="reset" onclick="sil();" class="btn btn-brick btn-xs large-padding pull-left"
                    style="margin-right: 2px;">Temizle
            </button>
            <button type="yazdir" onclick="print();" class="btn btn-brick btn-xs large-padding pull-left">Yazdır
            </button>
        </div>
    </div>
    <br/>
...
{!! Asset::add(Theme::url('js/calculator.js'), 'f-middle') !!}